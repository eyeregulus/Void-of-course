enum NoticeShowPolicy {
  /// [실제 서비스 배포용] 공지 ID(noticeId)가 바뀌었을 때 유저당 딱 1회만 노출합니다.
  /// 유저가 한 번 닫기 버튼을 누르면 noticeId를 새 값(v3 등)으로 수정하기 전까지 절대 다시 뜨지 않습니다.
  oncePerId,

  /// [개발 및 빌드 테스트용] 앱을 실행하거나 빌드할 때마다 매번 공지를 노출합니다.
  /// 공지 팝업의 디자인, 오타, 다국어 전환 등을 반복해서 눈으로 확인하고 싶을 때 이 값을 사용하세요.
  everyLaunch,
}
...
class AppNoticeConfig {
  ...
  /// 공지 식별 ID (이 값을 변경하면 'oncePerId' 정책일 때 유저들에게 다시 공지가 뜹니다)
  /// 예: 새 공지를 띄우고 싶을 때 ios_launch_event_v"n" 형태로 값을 변경하면 모든 유저에게 다시 팝업이 뜹니다.
  static const String noticeId = 'ios_launch_event_v3';

  /// 노출 정책 (한 번만 보여줄지, 켤 때마다 보여줄지 설정)
  /// - NoticeShowPolicy.oncePerId: 실제 서비스용 (유저당 1회 노출)
  /// - NoticeShowPolicy.everyLaunch: 개발/테스트용 (빌드할 때마다 매번 노출)
  static const NoticeShowPolicy showPolicy = NoticeShowPolicy.oncePerId;

  /// 공지 노출 만료일 (이 시각이 지나면 공지가 자동으로 뜨지 않습니다)
  /// 형식: 'YYYY-MM-DD HH:MM:SS' (빈 값 '' 설정 시 기간 제한 없이 상시 노출)
